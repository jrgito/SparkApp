...
object Utils {
  ...
  implicit def safeMap(option: Option[Map[String, String]]): Map[String, String] =
    option.getOrElse(Map[String, String]())

  implicit def safeSeq[T](option:Option[Seq[T]]) : Seq[T] = {
    option.getOrElse(Seq[T]())
  }
  /**
